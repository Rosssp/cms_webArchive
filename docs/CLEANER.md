...
5. ⚠️ **Стили грузятся через JS** (динамический сайт) — скрипт вырезан → стилей нет. Общий случай — 🤖.
   ✅ **Но частый подслучай ЗАКРЫТ детерминированно (2026-07-21, mddaonline.com):** темы HTML5 UP /
   templated.co на **skel.js** грузят CSS из JS (`skel.min.js`+`skel-layers.min.js`+`init.js` по
   брейкпоинтам), а настоящий `<link rel=stylesheet>` держат ТОЛЬКО в `<noscript>` как no-JS-фолбэк.
   Вырезали JS → грузить CSS некому, а браузер (JS вкл) `<noscript>` ИГНОРИРУЕТ → страница
   ПОЛНОСТЬЮ голая (`document.styleSheets` пуст, 3 `<link>` мёртво лежат в `<noscript>`). Наш
   восстановленный сайт — фактически no-JS, значит его `<noscript>`-фолбэк и есть правильный:
   `promote_noscript_stylesheets` (рядом с `strip_base_href`/`strip_sri_attrs`) выносит все
   `<link rel=stylesheet>`/`<style>` из `<noscript>` в `<head>` (по порядку, дубли по `href`
   отбрасываются; текст «включите JS» и пиксели в `<noscript>` не трогаются). Проверено рендером:
   mdda из голого HTML → полностью стилизован. **Регресс:** инертен без noscript-CSS; на Shopify
   Dawn (async-CSS `media=print onload` + noscript-фолбэк) выносит 12 фолбэков — speedgifts
   рендерится идентично, render-QA broken=False (async-onload всё равно режется, фолбэк лишь
   страхует). Аддитивно — фикс общего класса «CSS спрятан в `<noscript>`, JSа больше нет».
...
- ✅ **CSS-scroll слайдеры (Shopify Dawn) ОЖИВЛЯЕМ, не оставляем статикой** (владелец, 2026-07-21 —
  РАСШИРЯЕТ принцип выше для этого класса). `<slideshow-component>`/`<slider-component>` + трек
  `[id^="Slider-"]` (div/ul) — это CSS scroll-snap карусель: разметка И CSS целы, листание делал
  `global.js` темы (выброшен). `inject_slider_driver` дописывает КРОШЕЧНЫЙ ванильный драйвер
  (`index_files/_slider-driver.js`), который **перевешивает существующие** стрелки
  `.slider-button--prev/next`, точки `.slider-counter__link--dots` и добавляет мягкий автоплей — трек
  листается родным скроллом (`scrollBy(delta)`), разметка НЕ трогается (docs A1: не переписывать).
  Грабли, всплывшие при отладке: (а) трек — `<div>`, не `<ul>` (селектор ловит по `[id^="Slider-"]`);
  (б) Dawn чередует реальные `.slideshow__slide` с пустыми спейсерами w:0 — брать слайды по классу,
  иначе `delta=0`; (в) `scroll-behavior:smooth` — читать позицию после паузы. Гейт СТРОГО на
  Dawn-сигнатуру → в 13 рабочих сайтах её нет, аддитивно. Проверено на speedgifts: клик next →
  scrollLeft 0→1440, активная точка 0→1, скрин показывает 3-й слайд. owl/swiper/embla/slick — свои
  адаптеры позже, каждый проверять на живом сайте, чтобы рабочий набор не регрессировал.
...
16. ✅ Скачущие уровни заголовков (h2→h4). `normalize_heading_levels`.
    **Правило владельца (2026-07-21):** заголовки — ОПЦИОНАЛЬНО, свич на карточке лендинга «менять
    заголовки». **Дефолт СМЕНЁН на ВЫКЛ (2026-07-21, отменяет прежний «по умолчанию ВКЛ»)** — по
    умолчанию заголовки НЕ трогаем. Флип согласован в ДВУХ слоях: `templates/landing.html`
    (`_hdOn[id]===true?checked` и `nh=_hdOn[id]===true`) и `server.py` (дефолты `normalize_headings`
    в `_clean_entry`/`api_entry_clean` → `False`). Легаси одиночный STATE-режим не трогали. ВЫКЛ →
    `clean_html_file(..., normalize_headings=False)` →
...
16b. ✅ **Свич «не удалять пункты хедера» (`keep_header_items`).** Пункты меню хедера, которым некуда
    привязать якорь, при ВКЛ НЕ удаляются — у них просто снимается `href` (остаются текстом).
    `auto_link_menu(site_dir, keep_header_items=...)` — загейтлены ВСЕ 5 проходов удаления. **Дефолт
    СМЕНЁН на ВКЛ (2026-07-21)**: флип в `landing.html` (`_khOn[id]!==false?checked`, `kh=_khOn[id]
    !==false`) и `server.py` (дефолты `keep_header_items` → `True`).
...
38c. ✅ **СКАЧИВАНИЕ корёжило весь HTML → белая страница** (2026-07-21, speedgifts.ph — Shopify).
    Симптом владельца: «в вебархиве норм, качается норм, но после очистки бело, в body пусто». Корень
    НЕ в очистке — в ЗАГРУЗЧИКЕ (`wayback_download.py`, шаг «Переписываю ссылки на локальные»):
    `for url in sorted(url_to_local,...): html = html.replace(url, local)` — **слепой `str.replace`
    по всему HTML**. Среди ключей оказался вырожденный `"/"` (одиночный слэш): пойман ответ на голый
    `https://web.archive.org/`, а `url[len(host):]` даёт `"/"`. Замена `"/"` → `index_files/asset`
    прошлась по КАЖДОМУ слэшу документа: `text/javascript` → `textindex_files/assetjavascript`,
    `</head></body>` съедены → lxml не видит `<body>` → очистка отдаёт пустую белую страницу.
    Доказано реверсом мангла (замена `/`→`index_files/asset` даёт ровно наблюдаемое). **ДВА
    вырожденных ключа от одного мусорного ассета** (ответ на голый `https://web.archive.org/` —
    тулбар/редирект): (1) `"/"` → рушит структуру → белая страница; (2) `"https://web.archive.org/"`
    → заменяет архив-префикс в КАЖДОМ обёрнутом URL: `…/web/<ts>/https://cdn/x.js` →
    `index_files/assetweb/<ts>/https://cdn/x.js` → битый локальный путь → **десятки
    ERR_FILE_NOT_FOUND** (это то, что владелец увидел в консоли ПОСЛЕ первого фикса). **Фикс:**
    хелпер `_is_real_asset_ref(url)` — ключ валиден только если это `host+/web/<ts>` ИЛИ абсолютный
    site-URL (`http(s)`/`//`) ИЛИ root-relative `/web/…`; голый архив-хост-корень и `/`/пусто
    отсекаются. Проверено: body есть, `index_files/assetweb`=0, битых локальных ref=0, wayback-остатков=0,
    рендер рабочий; регресс — sanjha новым загрузчиком качается чисто. **КЛАСС:** «слепой str.replace
    словарём URL→local с вырожденным ключом рушит документ» — фикс аддитивный (фильтр — чистое
    подмножество, легит-ключи 13+ сайтов не тронуты).
